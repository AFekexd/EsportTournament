...
export function TimeSlotList({
    ...
    const handleAssignSupervisor = async (hour: number, slotKey: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (!user) return;
        try {
            setIsAssigning(slotKey);
    ...
    const handleRemoveSupervisor = async (id: string, slotKey: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (!user) return;
        try {
            setIsAssigning(slotKey);
                    ...
                    return (
                        <div key={slotKey} className="relative group">
                            ...
                            {user && !slot.isPast && config.needsSupervisor && (
                                <div className="mt-2 flex flex-col gap-2 relative" ref={openDropdown === slotKey ? dropdownRef : null}>
                                    {isAdmin && (
                                        <div className="px-1 relative">
                                            <div
                                                className={`w-full bg-background/50 border ${openDropdown === slotKey ? 'border-primary' : 'border-border hover:border-primary/50'} rounded-lg text-xs px-3 py-2 cursor-pointer flex items-center justify-between transition-colors`}
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setOpenDropdown(openDropdown === slotKey ? null : slotKey);
                                                }}
                                            >
                                                ...
                                                <ChevronRight
                                                    size={14}
                                                    className={`text-muted-foreground transition-transform duration-200 ${openDropdown === slotKey ? 'rotate-90 text-primary' : ''}`}
                                                />
                                            </div>

                                            {/* Dropdown Options List */}
                                            {openDropdown === slotKey && (
                                    ...
                                    <button
                                        onClick={(e) => handleAssignSupervisor(slot.hour, slotKey, e)}
                                        disabled={isAssigning === slotKey}
                                    ...
                                    <button
                                        onClick={(e) => {
                                            if (window.confirm(config.isMySupervision ? 'Biztosan lemondasz az ügyeletedről?' : 'Biztosan eltávolítod a kijelölt felelőst?')) {
                                                handleRemoveSupervisor(slot.supervisor!.id, slotKey, e);
                                            } else {
                                                e.stopPropagation();
                                            }
                                        }}
