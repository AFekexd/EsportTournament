...
import { Calendar, Clock, Monitor, Trash2, QrCode, CheckCircle, AlertCircle, Edit2 } from 'lucide-react';
import { useAppDispatch, useAppSelector } from '../../hooks/useRedux';
import { fetchMyBookings, deleteBooking, type Booking } from '../../store/slices/bookingsSlice';
...
export function MyBookings({ onEditBooking }: MyBookingsProps) {
    ...
    const getBookingStatus = (booking: Booking): 'upcoming' | 'active' | 'checked-in' => {
        const now = new Date();
                            ...
                            <div className="booking-card-actions">
                                {booking.checkInCode && status !== 'checked-in' && (
                                    <button
                                        ...
                                        onClick={() => setShowQRCode(showQRCode === booking.id ? null : booking.id)}
                            ...
                            {showQRCode === booking.id && booking.checkInCode && (
                                <div className="qr-code-panel">
                                    <div className="qr-code-placeholder">
                                        <QrCode size={64} />
                                        <p className="qr-code-value" style={{ wordBreak: 'break-all', fontFamily: 'monospace', fontSize: '0.85rem' }}>{booking.checkInCode}</p>
                                        <p className="qr-hint">Mutasd meg ezt a kódot a helyszínen</p>
                                    </div>
                                </div>
                            )}
                        </div>
