...
export function TournamentStandings({ tournament, onMatchClick }: TournamentStandingsProps) {
        ...
        if (match.homeTeamId) {
            ...
            return (
                <div className="flex items-center gap-2">
                    {team?.logoUrl ? (
                        <img src={team.logoUrl} alt={team.name} className="h-6 w-6 rounded-sm object-cover" />
                    ) : (
                        <div className="h-6 w-6 rounded-sm bg-primary/20" />
                    )}
                    <span className={type === 'home' && match.winnerId === match.homeTeamId ? 'font-bold text-primary' :
                        type === 'away' && match.winnerId === match.awayTeamId ? 'font-bold text-primary' : ''}>
                        {team?.name || 'TBD'}
                    </span>
                </div>
            );
            ...
            return (
                <div className="flex items-center gap-2">
                    ...
                    <span className={type === 'home' && match.winnerUserId === match.homeUserId ? 'font-bold text-primary' :
                        type === 'away' && match.winnerUserId === match.awayUserId ? 'font-bold text-primary' : ''}>
                        {user?.displayName || user?.username || 'TBD'}
                    </span>
                </div>
            );
        }
                                    ...
                                    <table className="w-full caption-bottom text-sm">
                                        <thead className="[&_tr]:border-b">
                                            <tr className="border-white/5 hover:bg-transparent transition-colors data-[state=selected]:bg-muted">
                                                <th className="h-12 px-4 text-left align-middle font-medium text-gray-400 w-[50px]">#</th>
                                                <th className="h-12 px-4 text-left align-middle font-medium text-gray-400">Résztvevő</th>
                                                <th className="h-12 px-4 text-center align-middle font-medium text-gray-400">M</th>
                                                <th className="h-12 px-4 text-center align-middle font-medium text-gray-400">GY</th>
                                                <th className="h-12 px-4 text-center align-middle font-medium text-gray-400">D</th>
                                                <th className="h-12 px-4 text-center align-middle font-medium text-gray-400">V</th>
                                                <th className="h-12 px-4 text-center align-middle font-bold text-primary">P</th>
                                            </tr>
                                        </thead>
                                        <tbody className="[&_tr:last-child]:border-0">
                                            {standings.map((team, index) => (
                                                <tr key={team.id} className="border-b border-white/5 hover:bg-white/5 transition-colors data-[state=selected]:bg-muted">
                                                    <td className="p-4 align-middle font-medium text-white">{index + 1}.</td>
                                                    <td className="p-4 align-middle">
                                                        <div className="flex items-center gap-3">
                                                            {team.avatarUrl ? (
                                                                <img src={team.avatarUrl} alt={team.name} className="h-8 w-8 rounded bg-gray-800 object-cover" />
                                                            ) : (
                                                    ...
                                                    <td className="p-4 align-middle text-center text-gray-300">{team.played}</td>
                                                    <td className="p-4 align-middle text-center text-green-400">{team.wins}</td>
                                                    <td className="p-4 align-middle text-center text-yellow-400">{team.draws}</td>
                                                    <td className="p-4 align-middle text-center text-red-400">{team.losses}</td>
                                                    <td className="p-4 align-middle text-center font-bold text-primary text-lg">{team.points}</td>
                                                </tr>
                                            ))}
                                            {standings.length === 0 && (
                                                <tr>
                                                    <td colSpan={7} className="p-4 align-middle h-24 text-center text-gray-400">
                                                        Nincsenek adatok
                                                    </td>
                                                </tr>
                                            )}
                                    ...
                                    {tournament.matches && tournament.matches.length > 0 ? (
                                        tournament.matches.map((match) => (
                                            <div
                                                ...
                                                className={`flex items-center justify-between p-4 rounded-lg bg-black/20 border border-white/5 hover:border-primary/30 transition-all ${onMatchClick ? 'cursor-pointer hover:bg-white/5' : ''}`}
                                            >
                                                ...
                                                <div className="flex flex-col items-center px-6 min-w-[100px]">
                                                    ...
                                                    <div className="text-xs text-gray-500 mt-1 uppercase tracking-wider">
                                                        {match.status === 'COMPLETED' ? 'Vége' : 'Folyamatban'}
                                                    </div>
                                                </div>
