...
function getParticipantName(match: Match, side: "home" | "away"): string {
  if (side === "home") {
    ...
    return match.homeTeam?.name || "Ismeretlen";
  } else {
    if (match.awayUser) {
      return match.awayUser.displayName || match.awayUser.username;
    }
    return match.awayTeam?.name || "Ismeretlen";
  }
}
...
function BracketControls({
  ...
  return (
    <div className="bracket-controls">
      ...
      <button
        className="bracket-control-btn reset"
        onClick={onReset}
        title="Visszaállítás"
      >
        ↺
...
export function TournamentBracket({
  ...
  useEffect(() => {
    ...
    const calculateLines = () => {
      ...
      const grandFinalMatches =
        tournament.matches?.filter((m) => m.bracketType === "GRAND_FINAL") ||
        [];
    ...
    return (
      <div className="bracket-empty">
        ...
        <p className="max-w-md mx-auto">
          Kattints a "Bracket generálása" gombra a sorsolás elkészítéséhez. A
          rendszer automatikusan párosítja a versenyzőket az erősorrend (seed)
          alapján.
        </p>
      </div>
    );
  ...
  const rounds = Object.keys(upperByRound)
    .map(Number)
    .sort((a, b) => a - b);
  const lowerRounds = Object.keys(lowerByRound)
    .map(Number)
    .sort((a, b) => a - b);
          ...
          <div className="flex items-center gap-16">
            <div className="flex flex-col gap-12">
              <div className="bracket-section upper-bracket">
                {isDoubleElimination && (
                  <h3 className="bracket-section-title">🏆 Felső ág</h3>
                )}
                  ...
                  <div className="bracket-rounds">
                    {lowerRounds.map((round) => (
                      <div key={`lower-${round}`} className="bracket-round">
                        <div className="round-title">
                          {getRoundName(round, lowerRounds.length, true)}
                        </div>
                        <div className="round-matches">
