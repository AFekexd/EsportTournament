  ...
  return (
    <div className="group relative flex items-center gap-4 p-4 bg-gradient-to-br from-card to-background border border-border rounded-xl hover:border-primary/30 hover:shadow-[0_0_30px_rgba(var(--primary-rgb),0.1)] hover:scale-[1.02] transition-all duration-300 overflow-hidden">
      ...
      <div className="flex-1 min-w-0 z-10">
        ...
        >
          <h3 className="text-lg font-bold text-foreground mb-0.5 whitespace-nowrap overflow-hidden text-ellipsis group-hover:text-primary transition-colors flex items-center gap-2">
      ...
      {isOwner && !isCaptain && !isCurrentUser && (
        <button
          className="absolute top-2 right-2 p-2 text-muted-foreground hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 z-20"
          onClick={(e) => {
            e.preventDefault();
            onRemove();
          }}
