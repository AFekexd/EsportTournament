...
export function UserManagement() {
    ...
    const getRoleBadge = (role: string) => {
        ...
        const config = roleConfig[role] || { class: 'bg-gray-500/20 text-gray-400 border-gray-500/30', label: role };
        return (
            <span className={`px-2 py-1 rounded text-xs font-semibold border ${config.class}`}>
                {config.label}
            </span>
        );
    };
