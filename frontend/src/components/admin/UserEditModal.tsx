...
export const UserEditModal: React.FC<UserEditModalProps> = ({ user, onClose, onSuccess }) => {
    ...
    const handleResetAvatar = async () => {
        if (!confirm('Biztosan törlöd a felhasználó profilképét?')) return;
        setIsLoading(true);
        try {
            const { authService } = await import('../../lib/auth-service');
            const token = authService.keycloak?.token;
    ...
    const handlePasswordReset = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/admin/kiosk/users/${user.id}/reset-password`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        <button
                            ...
                            className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${!user.avatarUrl ? 'opacity-50 cursor-not-allowed border-white/5' : 'hover:bg-red-500/10 border-white/10 hover:border-red-500/30'}`}
                        >
                            <div className="flex items-center gap-3">
                                <ImageOff className={user.avatarUrl ? "text-red-400" : "text-gray-500"} size={20} />
                                <div className="text-left">
                                    <div className="text-white font-medium">Profilkép törlése</div>
                                    <div className="text-xs text-gray-500">Alapértelmezett visszaállítása</div>
                                </div>
                            </div>
                        </button>
                        ...
                        <div className="p-3 rounded-lg border border-white/10 bg-black/20">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div className="text-white font-medium">Új jelszó beállítása</div>
                            </div>
