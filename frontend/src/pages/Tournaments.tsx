...
const statusLabels: Record<string, { label: string; colors: string }> = {
    DRAFT: { label: 'Tervezet', colors: 'bg-gray-500/10 text-gray-400 border-gray-500/20' },
    REGISTRATION: { label: 'Regisztráció', colors: 'bg-green-500/10 text-green-400 border-green-500/20' },
    IN_PROGRESS: { label: 'Folyamatban', colors: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' },
    COMPLETED: { label: 'Befejezett', colors: 'bg-primary/10 text-primary border-primary/20' },
    CANCELLED: { label: 'Törölve', colors: 'bg-red-500/10 text-red-400 border-red-500/20' },
};
        ...
        >
            ...
            <div className="relative w-full h-48 overflow-hidden bg-[#0f1015]">
                ...
                <div className="absolute top-3 right-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold border backdrop-blur-md ${statusInfo.colors}`}>
                        {statusInfo.label}
                    </span>
                </div>
                ...
                <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10">
                    <span className="text-xs font-bold text-white">{tournament.game?.name}</span>
                </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 pb-4 border-t border-white/5 pt-4">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-xs font-medium text-white">{startDate.toLocaleDateString('hu-HU')}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-12 text-center relative">
                ...
                <p className="text-lg text-gray-400 max-w-2xl mx-auto">
                    Böngészd a közelgő és folyamatban lévő versenyeket, és mutasd meg a tudásod!
                </p>
            </div>
            ...
            <div className="mb-8 flex flex-col md:flex-row gap-4">
                ...
                <div className="flex-1">
                    <div className="relative flex items-center">
                        <Search size={18} className="absolute right-4 text-gray-500 pointer-events-none z-10" />
                        <input
                            type="text"
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {[...Array(6)].map((_, i) => (
                        <div key={i} className="bg-[#1a1b26] rounded-xl overflow-hidden border border-white/5 h-[320px] animate-pulse">
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-[#1a1b26]/50 rounded-2xl border border-white/5">
                    ...
                    <p className="text-gray-400">Próbálj más szűrőket vagy keresési feltételeket.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
