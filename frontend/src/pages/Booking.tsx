        ...
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-8 border-b border-white/10">
                <div className="flex gap-8">
                    <button
                        className={`flex items-center gap-2 px-1 py-3 border-b-2 font-medium transition-colors relative ${activeTab === 'booking'
                    ...
                    <button
                        className={`flex items-center gap-2 px-1 py-3 border-b-2 font-medium transition-colors relative ${activeTab === 'my-bookings'
                                ? 'border-primary text-primary'
                                : 'border-transparent text-gray-400 hover:text-gray-300'
                            }`}
                ...
                <>
                    ...
                    <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
                        <div className="flex bg-[#1a1b26] p-1 rounded-xl border border-white/5">
                            <button
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${viewMode === 'daily'
                            ...
                            <button
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${viewMode === 'weekly'
                                        ? 'bg-[#0f1015] text-white shadow-lg'
                                        : 'text-gray-400 hover:text-gray-300'
                                    }`}
                        ...
                        {viewMode === 'daily' && (
                            <div className="flex items-center gap-3 bg-[#1a1b26] px-4 py-2 rounded-xl border border-white/5">
                                ...
                                />
                                <span className="text-gray-400 text-sm min-w-[140px]">{formatDate(selectedDate)}</span>
                            </div>
                        )}
                    </div>
                        ...
                        <>
                            {!todaySchedule && (
                                <div className="flex flex-col items-center justify-center py-20 bg-[#1a1b26]/50 rounded-2xl border border-white/5">
                                    ...
                                    <h3 className="text-xl font-bold text-white mb-2">Nincs elérhető időpont</h3>
                                    <p className="text-gray-400">{dayNames[dayOfWeek]} napon nincs nyitva a gaming szoba.</p>
                                </div>
                            )}
                                        ...
                                        {availableSlots.map((hour) => (
                                            <div key={hour} className="flex border-b border-white/5 last:border-b-0">
                                                <div className="w-24 flex-shrink-0 bg-[#0f1015] border-r border-white/5 flex flex-col justify-center items-center py-4">
                                                    ...
                                                    <span className="text-xs text-gray-500">{hour}:00 - {hour + 1}:00</span>
                                                </div>
                                                    ...
                                                    {computersByRow.map((row, rowIndex) => (
                                                        <div key={rowIndex} className="flex gap-4">
                                                            {row.map((computer) => {
                                                                ...
                                                                return (
                                                                    <div key={computer.id} className="flex-1 min-w-[140px] relative bg-[#0f1015] rounded-lg p-3 border border-white/5">
                                                                        <div className="flex justify-between items-center mb-2 pb-2 border-b border-white/5">
                                                                            <span className="text-sm font-semibold text-gray-300">{computer.name}</span>
                                                                            <button
                                                                        ...
                                                                        >
                                                                            ...
                                                                            {(!isBooked && !isMaintained && !isOutOfOrder) && (
            ...
            {showBookingModal && selectedComputer && selectedStartHour !== null && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowBookingModal(false)}>
                    <div className="bg-[#1a1b26] rounded-2xl p-8 w-full max-w-md border border-white/10 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="mb-6">
                            <label className="block mb-3 font-medium text-white">Időtartam:</label>
