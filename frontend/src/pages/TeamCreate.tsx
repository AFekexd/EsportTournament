...
import { ArrowLeft, ArrowRight, Check, Users, FileText, Image as ImageIcon, Sparkles, Shield } from 'lucide-react';
...
const STEPS = [
    ...
    { id: 2, title: 'Megjelenés', icon: ImageIcon, description: 'Logó és stílus' },
...
export function TeamCreatePage() {
        ...
        switch (currentStep) {
                ...
                return (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                        <div className="text-center mb-8">
                            <h2 className="text-2xl font-bold text-white mb-2">Kezdjük az alapokkal</h2>
                            <p className="text-gray-400">Add meg a csapatod nevét és egy rövid leírást.</p>
                        </div>
                        ...
                        <div className="space-y-5">
                                ...
                                <div className="relative group">
                                    ...
                                    <div className="relative bg-[#1a1b26] rounded-xl flex items-center">
                                        ...
                                        <input
                                            id="name"
                                            type="text"
                                            className={`w-full bg-transparent text-white border-0 rounded-xl px-4 py-3.5 placeholder-gray-600 focus:ring-0 focus:outline-none transition-all ${errors.name ? 'text-red-400' : ''}`}
                                            value={formData.name}
                            ...
                            <div className="space-y-2">
                                <label htmlFor="description" className="text-sm font-medium text-gray-300 ml-1">
                                    Leírás
                                </label>
                                <div className="relative group">
                                    <div className="absolute -inset-0.5 bg-gradient-to-r from-primary to-purple-600 rounded-xl blur opacity-20 group-hover:opacity-100 transition duration-1000 group-hover:duration-200" />
            ...
            case 2:
                return (
                    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                        <div className="text-center mb-8">
                            <h2 className="text-2xl font-bold text-white mb-2">Csapat megjelenése</h2>
                            <p className="text-gray-400">Adj hozzá logót, hogy kitűnjetek a tömegből.</p>
                        </div>

                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label htmlFor="logoUrl" className="text-sm font-medium text-gray-300 ml-1">
                                    Logó URL (opcionális)
                                </label>
                                <div className="relative group">
                                    <div className="absolute -inset-0.5 bg-gradient-to-r from-primary to-purple-600 rounded-xl blur opacity-20 group-hover:opacity-100 transition duration-1000 group-hover:duration-200" />
                ...
                return (
                    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
                        <div className="text-center">
                            <h2 className="text-2xl font-bold text-white mb-2">Minden rendben?</h2>
                            <p className="text-gray-400">Ellenőrizd az adatokat a létrehozás előtt.</p>
                        </div>
                                ...
                                <div className="w-24 h-24 rounded-full bg-black/40 border border-white/10 overflow-hidden flex-shrink-0 shadow-xl">
                                    {formData.logoUrl ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center">
                                            <span className="text-3xl font-bold text-white/20">{formData.name.charAt(0)}</span>
                                        </div>
                                    )}
                                </div>
    ...
    return (
        <div className="min-h-screen relative py-8 px-4 sm:px-6 lg:px-8 overflow-hidden bg-[#0A0A0B]">
            {/* Ambient Background - improved for depth */}
            ...
            <div className="max-w-5xl mx-auto relative">
                ...
                <div className="text-center mb-12">
                    <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight mb-4 drop-shadow-2xl">
                        Hozd létre a <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-purple-400 to-pink-500">Legendát</span>
                    </h1>
                    <p className="text-lg text-gray-400 max-w-2xl mx-auto font-light">
                        Alapítsd meg saját csapatodat, toborozz tagokat és indulj el a dicsőség felé vezető úton.
                    </p>
                </div>
                ...
                <div className="bg-[#12131A] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
                    ...
                    <div className="w-full md:w-80 bg-[#0E0F15] border-b md:border-b-0 md:border-r border-white/5 p-8 relative overflow-hidden">
                        ...
                        <div className="relative space-y-8">
                            {STEPS.map((step, index) => {
                                ...
                                return (
                                    <div key={step.id} className="relative group cursor-pointer" onClick={() => isCompleted && setCurrentStep(step.id)}>
                                        {/* Connecting Line */}
                                        {index < STEPS.length - 1 && (
                                            <div className={`absolute left-5 top-14 bottom-0 w-0.5 h-16 transition-colors duration-500 ${isCompleted ? 'bg-primary' : 'bg-white/5'}`} />
                                        )}

                                        <div className={`flex items-start gap-4 transition-all duration-300 ${isActive ? 'translate-x-2' : ''}`}>
                                            <div className={`
                                            ...
                                            `}>
                                                {isCompleted ? <Check size={18} /> :
                                                    <step.icon size={18} className={isActive ? 'animate-bounce-subtle' : ''} />}
                                            </div>
                                            <div className="pt-1">
                                                <h3 className={`font-bold text-sm tracking-wide uppercase mb-1 transition-colors ${isActive ? 'text-white' : isCompleted ? 'text-gray-300' : 'text-gray-600'}`}>
                                                    {step.title}
                                                </h3>
                                                <p className={`text-xs transition-colors ${isActive ? 'text-primary/80' : 'text-gray-600'}`}>{step.description}</p>
                                            </div>
                                        </div>
                                    </div>
                                );
                    ...
                    <div className="flex-1 flex flex-col relative bg-gradient-to-br from-[#12131A] to-[#0A0A0B]">
                        ...
                        <div className="p-8 border-t border-white/5 bg-[#0E0F15]/50 flex items-center justify-between backdrop-blur-sm">
                            <button
                                onClick={handleBack}
                                className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-medium transition-all ${currentStep > 1 ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'opacity-0 pointer-events-none'}`}
                            >
                                <ArrowLeft size={18} />
                                Vissza
                            </button>
