...
import { Users, Search, Plus, Trophy, ArrowRight, UserPlus } from 'lucide-react';
        ...
        >
            ...
            <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex-1">
                    ...
                    {team.description && (
                        <p className="text-sm text-gray-400 line-clamp-1">{team.description}</p>
                    )}
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4 pb-4 border-b border-white/5">
                <div className="flex flex-col items-center gap-1 text-center">
                    ...
                    <span className="text-xs text-gray-400">{team.members?.length || 0} tag</span>
                </div>
                <div className="flex flex-col items-center gap-1 text-center">
                    <Trophy size={16} className="text-yellow-500" />
                    <span className="text-xs text-gray-400">{team._count?.tournamentEntries || 0} verseny</span>
                </div>
                <div className="flex flex-col items-center gap-1 text-center">
        ...
        <div className="container mx-auto px-4 py-8">
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                    {[...Array(8)].map((_, i) => (
                        <div key={i} className="bg-[#1a1b26] rounded-xl overflow-hidden border border-white/5 h-[280px] animate-pulse p-6">
                            <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-[#1a1b26]/50 rounded-2xl border border-white/5">
                    ...
                    <p className="text-gray-400">Próbálj más keresési feltételeket vagy hozz létre új csapatot.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                    {teams.map((team: Team) => (
            ...
            {/* Join Modal */}
            {showJoinModal && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowJoinModal(false)}>
                    <div className="bg-[#1a1b26] rounded-2xl p-8 w-full max-w-md border border-white/10 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        <h2 className="text-2xl font-bold text-white mb-2">Csatlakozás kóddal</h2>
