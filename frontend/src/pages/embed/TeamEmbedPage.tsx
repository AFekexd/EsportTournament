    ...
    return (
        <div className="h-screen w-full bg-[#0f1015] overflow-hidden flex flex-col relative group">
            ...
            <div className="relative z-10 flex-1 flex flex-col p-4 md:p-6 justify-center items-center text-center">
                ...
                <div className="flex items-center gap-4 text-sm font-medium bg-white/5 backdrop-blur-sm border border-white/5 rounded-full px-4 py-2 mb-6">
                    <div className="flex items-center gap-1.5 text-yellow-500">
                        <Trophy size={14} />
                        <span className="text-white font-bold">{currentTeam.elo}</span> ELO
                    </div>
                    <div className="w-px h-4 bg-white/10" />
                    <div className="flex items-center gap-1.5 text-blue-400">
                        <Users size={14} />
                        <span className="text-white font-bold">{currentTeam.members?.length || 0}</span> Tag
                    </div>
                </div>
                ...
                {currentTeam.members && currentTeam.members.length > 0 && (
                    <div className="flex -space-x-3 mb-6">
                        {currentTeam.members.slice(0, 5).map((member) => (
                            <div key={member.id} className="w-10 h-10 rounded-full border-2 border-[#0f1015] relative group/member">
                                <LazyImage
            ...
            <div className="relative z-10 p-3 text-center">
                <a href="/" target="_blank" className="text-[10px] text-gray-500 hover:text-white uppercase tracking-widest font-bold opacity-60 hover:opacity-100 transition-opacity">
